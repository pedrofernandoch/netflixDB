	...
	class SplashScreen extends Thread {
		...
		public void run() {
			try {
				...
				Platform.runLater(new Runnable() {
					...
					public void run() {
						...
						try { // Load MainView page
							...
							//Alerts.showAlert("IOException", "Erro ao carregar a p�gina principal", e.getMessage(),AlertType.ERROR);
						}
				...
				e.printStackTrace();
				//EventLog.newEventLog(new Date(), "Sistema", LogActivities.ERROR, e.getMessage(), LogTypes.ERROR);
				//Alerts.showAlert("InterruptedException", "Erro ao carregar a p�gina principal", e.getMessage(),AlertType.ERROR);
			}
		}
	...
	/*private void addAppToTray() {
		try {
			...
			if (!java.awt.SystemTray.isSupported()) {
				...
				JOptionPane.showMessageDialog(frame, "Esse sistema operacional n�o suporta system tray");
				Platform.exit();
			}
